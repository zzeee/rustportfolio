...
//        println!("verify_format1");
 ...
 Ok(()) }
    ...
    pub fn io_i16<'a>(&'a self) -> Result<IO<'a, i16>> { println!("IOS");self.io_checked() }
...
impl<'a, S: Copy> IO<'a, S> {
    ...
    pub fn readi(&self, buf: &mut [S]) -> Result<usize> {
        //let bf2=::std::os::raw::c_void::new();
        println!("!!!readi {:?}",buf.len());
        println!("!!!readi22 {:?}",self.to_frames(buf.len()));
        //  snd_pcm_readi((self.0).0, buf.as_mut_ptr() as *mut c_void, self.to_frames(buf.len()))).map(|r| r as usize
